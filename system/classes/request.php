...
class Request {
	...
	public static function detect_uri()
	{
		...
		else
		{
			...
			else
			{
				...
				throw new CE('Unable to detect the URI using PATH_INFO, REQUEST_URI, PHP_SELF or REDIRECT_URL');
			}
			...
			if (Core::$index_file AND strpos($uri, Core::$index_file) === 0)
			{
				...
				$uri = substr($uri, strlen(Core::$index_file));
			}
		}
	...
	public function send_file($filename, $download = NULL, array $options = NULL)
	{
		...
		if ($filename === TRUE)
		{
			if (empty($download))
			{
				throw new CE('Download name must be provided for streaming files');
			}
		...
		if ( ! is_resource($file))
		{
			throw new CE('Could not read file to send: :file', array(
				':file' => $download,
			));
		}
	...
	public function execute()
	{
		...
		try
		{
			...
			if ($class->isAbstract())
			{
				throw new CE('Cannot create instances of abstract :controller',
