...
class Core {
	 ...
	 * `string`  | error_view | set the error rendering view                   | `"error"`
	 *
	 * @throws  Exception
	...
	public static function init(array $settings = NULL)
	{
		...
		if (isset($settings['error_view']))
		{
			...
			{
				throw new CE('Error view file does not exist: views/:file', array(
		...
		if (isset($settings['cache_dir']))
		{
			if ( ! is_dir($settings['cache_dir']))
			{
				...
				{
					throw new CE('Could not create cache directory :dir',
						array(':dir' => Core::debug_path($settings['cache_dir'])));
				}
			}
		...
		{
			throw new CE('Directory :dir must be writable',
				array(':dir' => Core::debug_path(Core::$cache_dir)));
		}
	...
	/**
	 * Provides auto-loading support of Core classes, as well as transparent
	...
	public static function find_file($dir, $file, $ext = NULL, $array = FALSE)
	{
		...
		if (Core::$caching === TRUE AND isset(Core::$_files[$path]))
		{
			// This path has been cached
			return Core::$_files[$path];
		}
		...
		if ( ! isset($messages[$file]))
		{
			...
			if ($files = Core::find_file('messages', $file))
			{
				foreach ($files as $f)
				{
					...
					$messages[$file] = Arr::merge($messages[$file], Core::load($f));
				}
			}
		}
