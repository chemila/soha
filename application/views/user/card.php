<div>
    <dl class="name clearFix">
        <dt>
            <a href="home/profile/<!--{$user.uid}-->" target="_blank"><img src="<!--{$user.portrait|fix_portrait}-->" imgtype="head" uid="<!--{$user.uid}-->" title="<!--{$user.nick}-->" class="picborder_r" style="width:50px; height:50px;"></a>
        </dt>
        <dd class="name_card_con0">
            <a href="home/profile/<!--{$user.uid}-->" title="<!--{$user.nick}-->" uid="<!--{$user.uid}-->" target="_blank"><!--{$user.nick}--><!--{if $user.verified|default:false}--><img class="small_icon vip" src="media/img/common/transparent.gif" > <!--{/if}--></a>
	...
	<!--{if not $is_self|default:false}-->
    <div class="links">
    	<!--没有登录用户不显示-->
	        <!--{if $followed|default:false}-->
	        ...
	        <!--{else}--> 
	        <a href="javascript:void(0);" onclick="App.userCardFollow(<!--{$user.uid}--> ,this);if(GB_SUDA&amp;&amp;(parseInt(Math.random()*10000)%1000 &lt; 10))GB_SUDA._S_uaTrack('tblog_userprofile_layer','userCardFollow');" class="btn_add"><img class="SG_icon" src="media/img/common/transparent.gif" title="加关注"><em>加关注</em></a>
	        <!--{/if}--> 
	        <span class="name_card_con3">
	            <i class="com_mess">&nbsp;</i>
	            <a href="javascript:void(0)" onclick="if(parseInt(Math.random()*10000)%1000 &lt; 10){GB_SUDA._S_uaTrack('tblog_userprofile_layer','msgDialog')};App.msgDialog('<!--{$user.nick}-->', <!--{$user.uid}-->);">私信</a>
	        </span>
        <!--
        <i>|</i>
        <span class="name_card_con3">
