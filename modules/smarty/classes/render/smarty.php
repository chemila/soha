...
public static function get_smarty() {
  ...
  try {
    ...
    throw new CE('Could not load Smarty class file');
  }
...
private static function create_dir($path, $name='') {
  if ( file_exists($path) ) {
    if ( is_dir($path) ) {
      throw new CE('Could not write to :name directory',
        array('name' => $name));
    } else {
      throw new CE(':name path is a file',
        array('name' => $name));
    }
  } else {
    try {
      ...
      throw new CE('Could not create :name directory',
        array('name' => $name));
    }
    if ( !is_writeable($path) ) {
      throw new CE('Created :name directory but could not write to it',
        array('name' => $name));
    }
  }
}
