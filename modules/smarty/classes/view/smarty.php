...
public function __construct($file = null, array $data = null) {
  $this->_config = Core::config('render');
...
public function _set_filename($file) {
  ...
  if ( array_key_exists($this->_renderer, $this->_config['extensions']) ) {
    ...
    if ( $ext===true ) {
      ...
      $path = true;
    } elseif ( ($path = Core::find_file('views', $file, $ext)) === false ) {
      throw new View_Exception('The requested view file :file.:ext could not be found', array(
        ':file' => $file, ':ext' => $ext,
      ));
    }
  } else { 
    throw new View_Exception('There is no extension set for the :renderer renderer',
